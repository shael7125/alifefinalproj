...
class Simulator:
    ...
    def set_constants(self):
        ...
        # new variables
        self.target_dx = ti.field(dtype=ti.f32, shape=())
        self.target_dy = ti.field(dtype=ti.f32, shape=())
        ...
        for sim_idx, spring_idx in ti.ndrange(self.n_sims[None], self.max_n_springs[None]):
            if spring_idx < self.n_springs[sim_idx]:
                ...
                spring_force = (
                    ...
                    * self.springK[None]
                    * dist
                    / (length + self.eps[None])
                )

                impulse = self.dt[None] * spring_force

                self.vinc[sim_idx, t+1, endpoint1] += -impulse
                self.vinc[sim_idx, t+1, endpoint2] += impulse

    @ti.kernel
    def apply_goal_force(self, t: ti.i32):
        for sim_idx, mass_idx in ti.ndrange(self.n_sims[None], self.max_n_masses[None]):

            if mass_idx < self.n_masses[sim_idx]:

                pos = self.x[sim_idx, t, mass_idx]

                dx = self.target_x - pos.x
                dy = self.target_y - pos.y

                dir = ti.Vector([dx, dy])
                dir = dir / (dir.norm() + 1e-6)   # normalize (constant strength)

                goal_force = 0.3 * dir            # adjust strength here

                impulse = self.dt[None] * goal_force

                self.vinc[sim_idx, t+1, mass_idx] += impulse
    ...
    #         com = self.center[sim_idx, self.steps[None]]
    #         dx = com.x - self.target_x
    #         dy = com.y - self.target_y
    ...
    #             com = self.center[sim_idx, t]
    #             dx = com.x - self.target_x
    #             dy = com.y - self.target_y
    #             dist = ti.sqrt(dx*dx + dy*dy)
    #             total_dist += dist  # sum over all timesteps
    #         # store the total distance as the loss
    #         self.loss[sim_idx] = total_dist
